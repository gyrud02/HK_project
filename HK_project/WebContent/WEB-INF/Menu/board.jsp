...
<%
	...
	if(count > 0){ // 만약 QnABoard테이블에 존재하는 글이 있다면
		list = boardDAO.getBoardList(startRow, pageSize); // QnABoard테이블에 존재하는 글을 가져온다.
...
%>		
		<tr height="35" onclick="location.href='../board/content.jsp?num=<%=boardBean.getNum()%>&pageNum=<%=pageNum%>'">
	        <td width = "50" align = "center"><%=boardBean.getNum()%></td>
			<td width = "360" align = "center">
<% 			
			int wid = 0; // 답변글에 대한 들여쓰기 값 저장
			
			if(boardBean.getRe_lev() > 0){ // 답변글에 대한 들여쓰기 값이 0보다 크므로 
				// 들여쓰기 값 처리
...
%>			
	        ...
	        <td width = "150" align = "center"><%=boardBean.getName()%></td>
	        <td width = "160" align = "center"><%=new SimpleDateFormat("yyyy.MM.dd").format(boardBean.getWritedate())%></td>
	        <td width = "70" align = "center"><%=boardBean.getReadcount()%></td>
        </tr>	
<%
		} // for문 끝
